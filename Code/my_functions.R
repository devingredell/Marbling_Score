...
density <- function(y_var, data){
  data %>%
    ggplot(aes(x = {{ y_var }}, group = group, fill = group)) +
    geom_density(alpha = 0.8)
}


box_plot <- function(y_var, data){
  data %>%
    ggplot(aes(x = group, y = {{ y_var}},
               group = group)) +
...
cor_plot <- function(x_var, y_var, data) {
  data %>%
...
mean_density_plot <- function(y_var, data, title = NULL,
  ...
  # custom tick labels
  if(y_var == "overall_sensory_experience"){
    left_anchor <- "0\nNegative"
    middle_anchor <- "\nNeutral"
    right_anchor <- "15\nPositive"
  ...
  if(y_var %in% c("overall_sensory_experience", "tenderness")){
    vline <- 7.5
...
tyson_theme <- function(){

  theme_minimal() +
    theme(
      text = element_text(color = "#002453", family = "Calibri",
                          face = "plain", size = 18),
      plot.title = element_text(size = 25, color = "#002453"),
      strip.text = element_text(color = "#002453", family = "Calibri",
                                size = 15,  hjust = 0.5),
      strip.background = element_rect(fill = "#ECF3F8", color = NA),
      axis.title.y = element_text(angle = 0, vjust = 0.5),
      axis.text = element_text(color = "#002453", family = "Calibri",
                               face = "plain", size = 15),
